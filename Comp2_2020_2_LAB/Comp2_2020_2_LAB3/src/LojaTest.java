...
public class LojaTest {
  ...
  public void setUp() {
    ...
    guinessBook = new Livro("Guiness Book Of Records", "Livro que contem os maiores recordes mundiais", 50f, new Dimensoes(), "Editora Abril");

    cuboMagico = new Brinquedo("Cubo Mágico", "Brinquedo tipo quebra cabeças", 12.5f, new Dimensoes());
  }

  @Test
  public void testarInclusaoDeProdutoNoEstoque() {
    loja.incluirProduto(guinessBook, 100);
    assertEquals("O estoque deve ser atualizado após cada venda", 100, loja.informarQuantidadeEmEstoque(guinessBook));

    loja.incluirProduto(guinessBook, 6);
    assertEquals("O estoque deve ser atualizado após cada venda", 106, loja.informarQuantidadeEmEstoque(guinessBook));
  }
  ...
  public void testarVendaBemSucedida() {
    ...
    assertEquals("Recibo no valor de R$ 250" + sep + "00 para Fulano "
        + "referente à compra de 5 unidades de Livro: Guiness Book Of Records", recibo.toString());

    assertEquals("O estoque deve ser atualizado após cada venda", 95, loja.informarQuantidadeEmEstoque(guinessBook));
  }
  ...
  public void testarLojaVendendoProdutosDiversos() {
    ...
    // não é preciso asserts, apenas está aqui para ver que isso precisa compilar
    // corretamente
  }
}