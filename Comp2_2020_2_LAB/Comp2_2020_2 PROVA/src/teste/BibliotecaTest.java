...
public class BibliotecaTest {
  ...
  public void testeAdquirirLivros() {
    ...
    assertEquals("A aquisição de um livro deve atualizar a quantidade daquele livro na estante", 10,
        biblioteca.getQuantidadeDeLivrosNaEstante(novoLivro));
    assertEquals("A aquisição de um livro deve atualizar a quantidade total de livros na estante", 16,
        biblioteca.getQuantidadeDeLivrosNaEstante());
  }

  @Test
  public void testeGetQuantidadeDeLivrosNaEstanteParaLivroInexistente() {
    assertEquals("A quantidade na estante de um livro inexistente deve ser reteornada como zero", 0,
        biblioteca.getQuantidadeDeLivrosNaEstante(livroInexistente));
  }

  @Test
  public void testeGetUsuario() {
    assertEquals("Um usuário cadastrado deve ser encontrável a partir de seu CPF", usuario1,
        biblioteca.getUsuario(cpfUsuario1));
  }

  @Test
  public void testeChavePrimariaUsuario() {
    Usuario novaInstanciaDoMesmoUsuario = new Usuario("Qualquer Nome", cpfUsuario1);
    assertEquals("Um usuário deve ser identificado exclusivamente pelo seu CPF", novaInstanciaDoMesmoUsuario, usuario1);
  }
  ...
  public void testeCadastrarNovoUsuario() {
    ...
    assertEquals("O cadastro de um novo usuario deve atualizar o total de usuários cadastrados", 2,
        biblioteca.getTotalDeUsuariosCadastrados());
  }
  ...
  public void testeCadastrarUsuarioJaCadastrado() {
    ...
    assertEquals("O cadastro de um usuario já cadastrado não deve atualizar o total de usuários cadastrados", 1,
        biblioteca.getTotalDeUsuariosCadastrados());
    Usuario usuarioRetornadoPelaBiblioteca = biblioteca.getUsuario(cpfUsuario1);
    assertEquals("O cadastro de um usuário já cadastrado deve atualizar seu nome", novoNome,
        usuarioRetornadoPelaBiblioteca.getNome());
    assertEquals("O cadastro de um usuário já cadastrado deve atualizar seu endereço", novoEndereco,
        usuarioRetornadoPelaBiblioteca.getEndereco());
  }
  ...
  public void testeEmprestarLivro()
    ...
    assertEquals("O empréstimo de um livro deve atualizar a quantidade de livros devidos por aquele usuário", 1,
        biblioteca.getQuantidadeDeLivrosDevidos(usuario1));
    assertEquals("O empréstimo de um livro deve atualizar a quantidade daquele livro na estante", 4,
        biblioteca.getQuantidadeDeLivrosNaEstante(livroAbundante));
  }
  ...
  public void testeDevolverLivro()
    ...
    assertEquals("A devolução de um livro deve atualizar a quantidade de livros devidos por aquele usuário", 0,
        biblioteca.getQuantidadeDeLivrosDevidos(usuario1));
    assertEquals("A devolução de um livro deve atualizar a quantidade daquele livro na estante", 5,
        biblioteca.getQuantidadeDeLivrosNaEstante(livroAbundante));
  }
  ...
  public void testeEmprestarLivroIndisponivel()
    ...
    assertFalse(
        "A tentativa de emprestar um livro com menos de " + Biblioteca.MIN_COPIAS_PARA_PODER_EMPRESTAR
  ...
  @Test
  public void testeEmprestarLivroParaUsuarioNaoCadastrado() throws LimiteEmprestimosExcedidoException {
    try {
      biblioteca.emprestarLivro(livroAbundante, usuarioFantasma);
      fail(
          "A tentativa de empréstimo para usuário não cadastrado deve lançar uma excecao.UsuarioNaoCadastradoException");
  ...
  public void testeEmprestarLivroParaUsuarioComMuitosLivrosDevidos()
    ...
    // garante que haverá cópias do livro suficientes (para este teste) nas estantes
    // da biblioteca
    ...
    // agora vamos tentar pegar emprestado ainda mais um livro --- não deve ser
    // permitido!
    try {
      biblioteca.emprestarLivro(livroAbundante, usuario1);
      fail("Um mesmo usuario nao pode pegar emprestado mais do que " + Biblioteca.MAX_LIVROS_DEVIDOS
